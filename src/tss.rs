...
pub fn step_two(
    ...
    let mut arr = [0u8; 32];
    arr.copy_from_slice(&*aggkey.agg_public_key.to_bytes(true));
    let aggpubkey = Pubkey::from(arr);
    let extended_kepair = ExpandedKeyPair::create_from_private_key(keypair.secret().to_bytes());
...
impl Signer for PartialSigner {
    fn try_pubkey(&self) -> Result<Pubkey, SignerError> {
        let mut arr = [0u8; 32];
        arr.copy_from_slice(&*self.aggregated_pubkey.agg_public_key.to_bytes(true));
        Ok(Pubkey::from(arr))
    }
...
mod tests {
    ...
    fn test_roundtrip() {
        ...
        let keys: Vec<_> = (0..n).map(|_| Keypair::new()).collect();
        let pubkeys: Vec<_> = keys.iter().map(|k| k.pubkey()).collect();
        // Key Generation
        let aggpubkey = key_agg(pubkeys.clone(), None).unwrap().agg_public_key;
        let mut arr = [0u8; 32];
        arr.copy_from_slice(&*aggpubkey.to_bytes(true));
        let aggpubkey_solana = Pubkey::from(arr);
