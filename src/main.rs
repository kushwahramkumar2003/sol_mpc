...
use solana_sdk::{native_token, signature::Signer};
...
fn main() -> Result<(), Error> {
    ...
    match opts {
        ...
        Options::SendSingle { keypair, amount, to, net, memo } => {
            let bytes = bs58::decode(&keypair).into_vec().map_err(Error::BadBase58)?;
            let keypair = Keypair::from_bytes(&bytes).map_err(|e| Error::WrongKeyPair(e.to_string()))?;
            let rpc_client = RpcClient::new(net.get_cluster_url().to_string());
